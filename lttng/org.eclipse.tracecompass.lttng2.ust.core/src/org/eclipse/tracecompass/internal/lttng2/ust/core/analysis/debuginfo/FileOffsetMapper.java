...
public final class FileOffsetMapper {
        ...
        CALLSITE_CACHE = checkNotNull(CacheBuilder.newBuilder()
            ...
            .build(new CacheLoader<FileOffset, @Nullable Iterable<SourceCallsite>>() {
                ...
                public @Nullable Iterable<SourceCallsite> load(FileOffset fo) {
                    LOGGER.info(() -> "[FileOffsetMapper:CacheMiss] file/offset=" + fo.toString()); //$NON-NLS-1$
    ...
    public static @Nullable Iterable<SourceCallsite> getCallsiteFromOffset(File file, String buildId, long offset) {
        LOGGER.fine(() -> String.format("[FileOffsetMapper:Request] file=%s, buildId=%s, offset=0x%h", //$NON-NLS-1$
                file.toString(), buildId, offset));

        if (!Files.exists((file.toPath()))) {
            LOGGER.fine(() -> "[FileOffsetMapper:RequestFailed] File not found"); //$NON-NLS-1$
            return null;
        }
        ...
        LOGGER.fine(() -> String.format("[FileOffsetMapper:RequestComplete] callsites=%s", callsites)); //$NON-NLS-1$
        return callsites;
    }
